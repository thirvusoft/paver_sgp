...
def execute(filters=None):
	...
	chart_data = {
		"data": {
			"labels": [[(i.get('vehicle') or ''), *(filters.get('item_group') or [])] for i in data],
			"datasets": [
...
def get_data(filters):
	return frappe.db.sql(f"""
		SELECT
		    CASE 
				WHEN IFNULL(dn.own_vehicle_no, '')!='' 
					THEN TRIM(dn.own_vehicle_no)
				ELSE
					TRIM(IFNULL(dn.vehicle_no, '-'))
			END as vehicle,
		      {
			      ", ".join([
				      	f'''
		...
		GROUP BY 
			CASE 
				WHEN IFNULL(dn.own_vehicle_no, '')!='' 
					THEN TRIM(LOWER(dn.own_vehicle_no))
				ELSE
					TRIM(LOWER(IFNULL(dn.vehicle_no, '-')))
			END
	""", as_dict=True)
