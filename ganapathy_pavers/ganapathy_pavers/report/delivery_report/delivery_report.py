...
class PartyLedgerSummaryReport(object):
	...
	def run(self, args):
		...
		for row in data:
			...
			if self.filters.get("sw_status") and row.get("project"):
				try:
					if (frappe.get_value("Project", row.get("project"), "status") in self.filters.get("sw_status")):
	...
	def prepare_conditions(self):
		...
		if self.filters.get('project'):
			...
			projects = tuple(frappe.get_all("Project", {"dont_show_in_delivery_report": 1}, pluck="name"))
			if len(projects) == 1:
				conditions.append("gle.project != '{0}'".format(projects[0]))
			elif len(projects) > 1:
				conditions.append(f"gle.project not in {projects}")
