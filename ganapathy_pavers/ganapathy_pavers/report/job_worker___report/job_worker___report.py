...
def execute(filters=None):
    ...
    if filters.get("group_site_work"):
        ...
        for idx in range(len(data)):
            ...
            if _key not in data1 or data[idx][4+1]:
                data1[_key]=data[idx]
            else:
                data1[_key][3]+=(data[idx][3] or 0)
                data1[_key][4]+=(data[idx][4] or 0)
                data1[_key][7+1]+=(data[idx][7+1] or 0)
    ...
    for row in data:
        if row[0] and frappe.db.exists("Employee", row[0]):
            row[6+1]=get_employee_salary_balance(employee=row[0], from_date=from_date, to_date=to_date)
    ...
    if(len(data)):
        ...
        for i in range(len(data)-1):
            if (data[i][0] != data[i+1][0]):
                ...
                total[4+1]=0
                total[6+1] = sum((data[i][6+1] or 0) for i in range(start,i+1))
                total[7+1] = f"<b>{'%.2f'%sum((data[i][7+1] or 0) for i in range(start,i+1))}</b>"
                total[8+1] = adv
                amount=sum((data[i][7+1] or 0) for i in range(start,i+1))
                salary_bal=sum((data[i][6+1] or 0) for i in range(start,i+1))
                total[9+1] = round(((amount or 0)+(salary_bal or 0)-(adv or 0)), 2)
                final_data[-1][6+1]=None
                ...
                data[i][6+1]=None
                data[i][9+1]=None
                if(c==0):data[i][8+1]=None
                else:data[i][8+1]=None
                c+=1
                final_data.append(data[i])
        adv=data[-1][8+1]
        data[-1][8+1]=None
        ...
        total[4+1]=0
        total[6+1] = sum((data[i][6+1] or 0) for i in range(start,len(data)))
        total[7+1] = f"<b>{'%.2f'%sum((data[i][7+1] or 0) for i in range(start,len(data)))}</b>"
        total[8+1] = adv
        amount=sum((data[i][7+1] or 0) for i in range(start,len(data)))
        salary_bal=sum((data[i][6+1] or 0) for i in range(start,len(data)))
        total[9+1] = round(((amount or 0)+(salary_bal or 0)-(adv or 0)), 2)
        final_data[-1][6+1]=None
        final_data.append(total)
    other_work=0
