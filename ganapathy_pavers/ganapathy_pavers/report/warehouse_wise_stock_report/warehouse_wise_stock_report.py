...
def execute(filters=None):
	...
	if filters.get("location"):
		war = frappe.get_value('Location', filters.get("location"), 'warehouse')
		filters['warehouse'] = war
	...
	# iwb_map.update(get_item_location_map(filters, sle))
...
def get_item_location_map(filters, sle):
	iwb_map = {}
	from_date = getdate(filters.get("from_date"))
	to_date = getdate(filters.get("to_date"))

	float_precision = cint(frappe.db.get_default("float_precision")) or 3
	v = frappe.get_all('Location', filters={'name':filters.get("location")}, pluck='warehouse')
	access = []	
	parent_grps = v
	
	for i in parent_grps:
		access.extend(frappe.get_all('Warehouse', filters={'parent_warehouse':['in', i]}, pluck='name'))
		parent_grps.extend(frappe.get_all('Warehouse', filters={'parent_warehouse':['in', i], 'is_group':1}, pluck='name'))
	access+=parent_grps
	for d in sle:
		for e in access:
			key = (d.company, d.item_code, e)
			if key not in iwb_map:
				iwb_map[key] = frappe._dict(
				{
					"bal_qty": 0.0,
		...
		qty_dict = iwb_map[(d.company, d.item_code, e)]
