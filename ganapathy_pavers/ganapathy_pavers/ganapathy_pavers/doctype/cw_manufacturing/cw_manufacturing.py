...
def make_stock_entry_for_bundling(doc):
    ...
    if doc.get("damaged_qty_at_bundling") > 0:
        scrap_qty = uom_conversion(doc.get('item_to_manufacture'), manufacture_uom, doc.get("damaged_qty_at_bundling"), default_nos)
        stock_entry.append('items', dict(
            t_warehouse = default_scrap_warehouse, item_code = doc.get("item_to_manufacture"),qty =  scrap_qty ,uom = manufacture_uom,  is_process_loss = 1
            ))
    stock_entry.append('additional_costs', dict(
