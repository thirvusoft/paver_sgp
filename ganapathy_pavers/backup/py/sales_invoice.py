...
# TS CUSTOMIZATION
	def update_billing_status_in_dn(self, update_modified=True):
		...
		for d in self.get("items"):
			if d.dn_detail:
				billed_amt = frappe.db.sql(
					"""select sum(amount),sum(stock_qty) from `tabSales Invoice Item`
					where dn_detail=%s and docstatus=1""",
					d.dn_detail,
				)
				
				total_billed_qty+=billed_amt and billed_amt[0][1] or 0
				billed_qty=billed_amt and billed_amt[0][1] or 0
				...
				frappe.db.set_value(
					"Delivery Note Item", d.dn_detail, "billed_qty", billed_qty, update_modified=update_modified
				)
				delivery_doc=frappe.get_doc("Delivery Note",d.delivery_note)
				# if round(delivery_doc.total_billed_qty ) == round(delivery_doc.total_stock_qty):
				delivery_doc.set_status(update=True, update_modified=False)
		...
		for dn in set(updated_delivery_notes):
			update_delivery_note_total_billed_qty(dn)
...
# TS CUSTOMIZATION
def update_delivery_note_total_billed_qty(dn):
	total_billed_qty=0
	total_stock_qty=0
	for i in frappe.get_doc("Delivery Note",dn).items:
		total_billed_qty+=i.billed_qty
		total_stock_qty+=i.stock_qty

	frappe.db.set_value(
					"Delivery Note", dn, "total_billed_qty", total_billed_qty, update_modified=False
				)
	frappe.db.set_value(
					"Delivery Note", dn, "total_stock_qty", total_stock_qty, update_modified=False
				)
