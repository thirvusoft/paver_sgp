"""erpnext/erpnext/stock/doctype/delivery_note/delivery_note.py"""
...
def get_invoiced_qty_map(delivery_note):
	...
	invoiced_qty_map = {}
	# TS CUSTOMIZATION
	for dn_detail, qty in frappe.db.sql("""select dn_detail, stock_qty from `tabSales Invoice Item`
...
def make_sales_invoice(source_name, target_doc=None):
	...
	def get_pending_qty(item_row):
		...
		if invoiced_qty_map.get(item_row.name, 0):
			invoiced_qty_map.get(item_row.name, 0) - item_row.qty
	...
	doc = get_mapped_doc("Delivery Note", source_name, {
		...
		"Delivery Note Item": {
			...
			"filter": lambda d: get_pending_qty(d) <= 0 if not doc.get("is_return") else get_pending_qty(d) >= 0 # TS CUSTOMIZATION
		},
