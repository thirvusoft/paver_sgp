...
def get_delivery_notes_to_be_billed(doctype, txt, searchfield, start, page_len, filters, as_dict):
	...
	# TS CUSTOMIZATION  
	# and `tabDelivery Note`.is_not_billable = 0
	# `tabDelivery Note`.per_billed < 100 >>>>>>>>> `tabDelivery Note`.total_billed_qty < `tabDelivery Note`.total_qty

	return frappe.db.sql("""
		select %(fields)s
		from `tabDelivery Note`
		where `tabDelivery Note`.`%(key)s` like %(txt)s and
			`tabDelivery Note`.docstatus = 1 and `tabDelivery Note`.is_not_billable = 0
			and status not in ("Stopped", "Closed") %(fcond)s
			and (
				(`tabDelivery Note`.is_return = 0 and `tabDelivery Note`.total_billed_qty < `tabDelivery Note`.total_qty)
				or (`tabDelivery Note`.grand_total = 0 and `tabDelivery Note`.total_billed_qty < `tabDelivery Note`.total_qty)
				or (
					`tabDelivery Note`.is_return = 1
					and return_against in (select name from `tabDelivery Note` where total_billed_qty < `tabDelivery Note`.total_qty)
				)
			)
