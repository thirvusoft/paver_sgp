...
frappe.views.ReportView = class ReportView extends frappe.views.ListView {
		...
		if (this.report_name) {
			return this.get_report_doc()
				.then(doc => {
					...
					this.average_columns = this.report_doc.json.average_columns || []; // Customization
			...
			this.average_columns = this.view_user_settings.average_columns || [] // Customization
		}
	}
		...
		this.datatable = new DataTable(this.$datatable_wrapper[0], {
			...
			{    // Customization Start
				...
				action: (datatabe_col) => {
					...
					if (this.average_columns.includes(datatabe_col.id)) {
						const index = this.average_columns.indexOf(datatabe_col.id);
						if (index > -1) { 
							this.average_columns.splice(index, 1);
						}
					} else {
						this.average_columns.push(datatabe_col.id)
					}

					this.save_view_user_settings({
						average_columns: this.average_columns
					});
	...
	get_columns_totals(data) {
		...
		if (this.average_columns && data.length) {  // Customization Start
			Object.keys(row_totals).forEach(field => {
				if (this.average_columns.includes(field)) {
					row_totals[field] = row_totals[field]/data.length
				}
			})
		} // Customization end
		return row_totals;
