"erpnext/erpnext/stock/doctype/delivery_note/delivery_note_list.js"

frappe.listview_settings['Delivery Note'] = {
	add_fields: ["customer", "customer_name", "base_grand_total", "per_installed", "per_billed",
		"transporter_name", "grand_total", "is_return", "status", "currency","total_billed_qty","total_stock_qty"],
	get_indicator: function(doc) {
		...
		} else if (Math.round(doc.total_billed_qty) != Math.round(doc.total_stock_qty)) {
			return [__("To Bill"), "orange","status,=,To Bill"];
		} 
		else if (Math.round(doc.total_billed_qty) == Math.round(doc.total_stock_qty) && (Math.round(doc.total_billed_qty) !=0 && Math.round(doc.total_stock_qty) !=0)) {
