...
class PartyLedgerSummaryReport(object):
	...
	def get_data(self):
		...
		for party, row in iteritems(self.party_data):
			if row.opening_balance or row.invoiced_amount or row.paid_amount or row.return_amount or row.closing_amount:
				if (self.filters.get("closing_balance_zero") and not round((row.get("closing_balance") or 0), 3) ):
					continue

				if self.filters.get("closing_balance_opt") == "Greater than 0" and round((row.get("closing_balance") or 0), 3) <= 0:
					continue

				if self.filters.get("closing_balance_opt") == "Less than 0" and round((row.get("closing_balance") or 0), 3) >= 0:
					continue

				if self.filters.get("closing_balance_opt") == "Equals 0" and round((row.get("closing_balance") or 0), 3) != 0:
					continue
	...
	def get_gl_entries(self):
		...
		if self.filters.get("sales_type") == "--Nothing--":
			sales_type_query = " and IFNULL(gle.type, '') = '' "
		elif self.filters.get("sales_type") in ("Compound Wall", "Compound Walls"):
			sales_type_query = """ and gle.type in ('Compound Wall', 'Compound Walls') """
		elif self.filters.get('sales_type'):
			sales_type_query = """ and gle.type=%(sales_type)s """
	...
	def get_party_adjustment_amounts(self):
		...
		if self.filters.get("sales_type") == "--Nothing--":
			sales_type_query = " and IFNULL(type, '') = '' "
		elif self.filters.get("sales_type") in ("Compound Wall", "Compound Walls"):
			sales_type_query = """ and type in ('Compound Wall', 'Compound Walls') """
		elif self.filters.get('sales_type'):
			sales_type_query = """ and type=%(sales_type)s """
		...
		gl_entries = frappe.db.sql(("""
			...
			from
				`tabGL Entry`
			where
				docstatus < 2 and is_cancelled = 0 """ + sales_type_query + """ 
