...
def get_conditions(filters):
	...
	if not filters.get("group_by"):
		conditions += "ORDER BY `tabPurchase Invoice Item`.type desc, `tabPurchase Invoice`.posting_date desc, `tabPurchase Invoice Item`.item_code desc" #customized
	else:
...
def get_items(filters, additional_query_columns):
	...
	""".format(additional_query_columns) % (conditions), filters, as_dict=1) #customized
